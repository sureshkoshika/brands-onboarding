...
-- No. of ASINS/ Brands enrolled month on month
...
where 
TRUNC(DATE_PART('day', CURRENT_DATE::timestamp - (published_date)::timestamp)/7) <=10
GROUP BY extract(week from(published_date)), extract(year from(published_date))
ORDER BY 4;

 
 
 --  asin level FEEDBACK STATUS
 ...
 SELECT  
  ...
  , CONCAT(COUNT
(brand_feedback)*100/
  (SELECT COUNT(b1.asin) FROM onboarded_asins b1),'%') AS ASIN_feedback_Percentage
FROM onboarded_asins
...
GROUP BY brand_region;
 ...
 , CONCAT(
	...
	*100/
  (SELECT  COUNT(distinct(brand_name)) FROM onboarded_asins b1),'%') AS brand_feedback_Percentage
FROM onboarded_asins
GROUP BY brand_feedback
ORDER BY brand_feedback;
...
--Brand/ASIN wise breakdown as per feedback status from respective brands:
...
-- old 5 weeks data query

SELECT 
 ('Week' || x.week_number) as Week_number
  , y.onboarded_asins
  , y.onboarded_brands
  , COALESCE(x.approved_asins,0) as Approved_asins
  , COALESCE(x.approved_brands,0) as Approved_brands
  , COALESCE(x.rejected_asins,0) as Rejected_asins
  , COALESCE(x.rejected_brands,0) as Rejected_brands
  , COALESCE(x.modification_requested_asins,0) as Modification_requested_asins
  , COALESCE(x.modification_requested_brands,0) as Modification_requested_brands
  , COALESCE(x.under_brand_review_asins,0) as Under_brand_review_asins
  , COALESCE(x.under_brand_review_brands,0) as Under_brand_review_brands
FROM
(WITH 
	 approved (week_number,approved_asins) as
	 (
...
select ur.week_number
	 ,a.approved_asins
     ,a.approved_brands
	 ,r.rejected_asins
     ,r.rejected_brands
	 ,rm.modification_requested_asins
     ,rm.modification_requested_brands
	 ,ur.under_brand_review_asins
     ,ur.under_brand_review_brands
	 from Under_brand_review ur 
     left join approved a
	 on ur.week_number=a.week_number
	 full outer join Require_modification rm
	 on ur.week_number=rm.week_number
	 full outer join Rejected r
	 on ur.week_number=r.week_number
	 ORDER BY ur.week_number ) x
JOIN
  (
   select  COUNT(asin) as onboarded_asins
	   , count(distinct(brand_name)) as Onboarded_brands
 ...
 ON x.week_number=y.week_number;
...
-- avg time spent on page, vs avg video play percentage
